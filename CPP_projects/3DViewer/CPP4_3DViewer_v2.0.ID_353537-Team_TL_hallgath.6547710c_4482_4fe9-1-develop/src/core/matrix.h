...
class S21Matrix {
 ...
 public:
  ...
  S21Matrix(std::initializer_list<std::initializer_list<double> > list)
      : S21Matrix(list.size(), list.begin()[0].size()) {
  ...
  class Proxy {
   public:
    ...
    double& operator[](int index) { return m_[index]; }

    double& operator[](int index) const { return m_[index]; }
  ...
  double** GetMatrixArray() { return matrix_; }
};
