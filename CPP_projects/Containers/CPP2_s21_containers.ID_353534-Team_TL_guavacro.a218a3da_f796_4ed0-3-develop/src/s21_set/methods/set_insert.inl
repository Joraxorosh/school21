...
inline void s21::set<Key, Compare>::fix_insert_violations(Node* insert_node) {
...
inline std::pair<typename s21::set<Key, Compare>::iterator, bool>
s21::set<Key, Compare>::insert(
    const typename s21::set<Key, Compare>::value_type& value) {
  using Node = typename s21::set<Key, Compare>::Node;
  Node* insert_node = new Node{value, s21::set<Key, Compare>::RED};
  Node* node_to_compare = this->m_root;
  Node* value_parent = Node::nil();
  Compare compare;

  while (node_to_compare != Node::nil()) {
    if (insert_node->value == node_to_compare->value) {
      delete insert_node;
      return {nullptr, true};
    }
  ...
  typename s21::set<Key, Compare>::iterator Iterator(insert_node);
  std::pair<typename s21::set<Key, Compare>::iterator, bool> returnedPair(
      Iterator, true);
