...
Поскольку обрабатывать необходимо в разных процессах, используй модуль `multiprocessing`.

Для получения случайных значений используй модуль `random`. Можно выбирать элементы из
...
Сортируй студентов и экзаменаторов с помощью функции `sorted` (или метода `sort`, если возникнет необходимость сортировать исходный список). В примере выше можно отсортировать точки по убыванию координаты `x` так:
